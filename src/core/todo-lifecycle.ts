...
import {
  ...
  type TodoLifecycleResultV1,
  type TodoPriorityV1,
  type TodoStatusV1,
} from "../contracts/v1/index.js";
import { exclusiveWrite, fsyncDirectory, type FailureInjector, removeIfMatches, withDirectoryLock, withMutationQueue, writeJsonAtomic } from "./atomic.js";
...
export type TodoRuntimeOptions = Readonly<{ failureInjector?: FailureInjector }>;
type TodoIssue = { path?: string; code: string; summary: string };
type Inspected = { state: "valid" | "invalid" | "conflict"; todo?: TodoIdentityV1; issues: TodoIssue[]; text?: string };
type ScanResult = { todos: TodoIdentityV1[]; issues: TodoIssue[]; directoryHash: string; rootExists: boolean };
type TodoTransitionJournal = {
  schema: "@aefree/pi-project-artifacts/todo-transition-journal";
  version: 1;
  state: "prepared" | "target_written" | "source_removed";
  source: string;
  target: string;
  sourceHash: string;
  targetHash: string;
...
export async function executeTodoLifecycle(
  context: ArtifactExecutionContextV1,
  request: TodoLifecycleRequestV1,
  options: TodoRuntimeOptions = {},
  ...
  if (request.operation === "inspect") return await inspectOperation(roots.todosRoot, path.resolve(request.path));
  ...
  return await withMutationQueue(roots.todosRoot, async () => await withDirectoryLock(lockPath, lockOwner(roots.todosRoot), async () => {
    ...
    const scan = await scanTodos(roots.todosRoot);
    if (request.operation === "allocate_id") return allocateOperation(request.expectedDirectoryHash, scan);
    if (request.operation === "create") return await createOperation(context, request, roots, scan, options.failureInjector);
...
async function resolveTodoRuntimeRoots(executionCwd: string, workspaceRaw: string, todosRaw: string | undefined, createTodos: boolean): Promise<{ workspaceRoot: string; todosRoot: string }> {
  ...
  try { todosRoot = await scopedPhysicalDirectory(physicalExecutionRoot, todosRoot, "todosRoot"); }
  catch (error) {
    if (!(error instanceof ProjectArtifactError) || error.code !== "root_missing") throw error;
    if (createTodos) {
...
async function inspectOperation(todosRoot: string, target: string): Promise<TodoLifecycleResultV1> {
...
function allocateOperation(expectedDirectoryHash: string | undefined, scan: ScanResult): TodoLifecycleResultV1 {
  const blocker = preExistingConflict(scan);
  if (blocker) return blocker;
  if (expectedDirectoryHash !== undefined && expectedDirectoryHash !== scan.directoryHash) return conflict("directory_changed", "The todos directory changed after its expected hash was captured.");
...
async function createOperation(
  context: ArtifactExecutionContextV1,
  request: Extract<TodoLifecycleRequestV1, { operation: "create" }>,
  roots: { workspaceRoot: string; todosRoot: string },
  scan: ScanResult,
  failureInjector?: FailureInjector,
): Promise<TodoLifecycleResultV1> {
  const blocker = preExistingConflict(scan);
  if (blocker) return blocker;
  if (request.expectedDirectoryHash !== undefined && request.expectedDirectoryHash !== scan.directoryHash) return conflict("directory_changed", "The todos directory changed after its expected hash was captured.");
  const issueId = Math.max(0, ...scan.todos.map((todo) => todo.issueId)) + 1;
  if (!Number.isSafeInteger(issueId)) return blocked("id_exhausted", "No safe numeric todo ID remains.");
  const renderedId = renderId(issueId);
  const status = request.todo.status ?? "pending";
  const slug = slugify(request.todo.title);
  if (slug === "") return blocked("description_invalid", "Todo title cannot produce a canonical filename description.");
  ...
  if ([...scan.todos, ...scan.issues.flatMap(() => [])].some((todo) => "path" in todo && typeof todo.path === "string" && collisionKey(todo.path) === collisionKey(target))) return conflict("path_collision", "The canonical todo target collides with an existing path.");
  const metadata = Object.assign(Object.create(null) as Record<string, unknown>, request.todo.frontmatter ?? {});
  ...
  return await withMutationQueue(target, async () => {
    ...
    if (targetWritten) {
      ...
      if (!removed) throw new ProjectArtifactError("rollback_conflict", `Create rollback refused to remove externally changed target: ${normalizePath(target)}`);
    }
    await unlink(stage).catch(() => undefined);
      if (hasCode(error, "EEXIST")) return conflict("target_exists", `Todo target already exists: ${normalizePath(target)}`);
...
async function transitionOperation(
  context: ArtifactExecutionContextV1,
  request: Extract<TodoLifecycleRequestV1, { operation: "transition" }>,
  roots: { workspaceRoot: string; todosRoot: string },
  scan: ScanResult,
  failureInjector?: FailureInjector,
): Promise<TodoLifecycleResultV1> {
  ...
  if (inspected.state !== "valid" || inspected.todo === undefined || inspected.text === undefined) return conflict("preexisting_todo_conflict", "The requested todo is not canonical and was left untouched.");
  const originalText = inspected.text;
  if (inspected.todo.contentHash !== request.expectedContentHash) return Object.freeze({ outcome: "conflict", code: "content_changed", summary: "Todo content changed after its expected hash was captured.", currentContentHash: inspected.todo.contentHash });
  ...
  await assertPhysicalContainment(roots.todosRoot, target, "todo target");
  if (scan.todos.some((todo) => collisionKey(todo.path) === collisionKey(target) && collisionKey(todo.path) !== collisionKey(source))) return conflict("path_collision", `Transition target collides with an existing todo: ${normalizePath(target)}`);
  ...
  return await withMutationQueue(source, async () => await withMutationQueue(target, async () => {
    ...
    const journal: TodoTransitionJournal = {
      schema: "@aefree/pi-project-artifacts/todo-transition-journal",
      version: 1,
      state: "prepared",
      source,
    ...
    try {
      ...
      await chmod(target, sourceMode).catch((error) => { if (process.platform !== "win32") throw error; });
      ...
      if (currentHash !== request.expectedContentHash) throw new ProjectArtifactError("content_changed", "Todo changed immediately before source removal.");
      await assertPhysicalContainment(roots.todosRoot, source, "todo source");
      await unlink(source);
      ...
      try { await rollbackTransitionRun(roots, runDirectory, journal); }
      catch (restoreError) { throw new ProjectArtifactError("rollback_failed", `Transition rollback could not restore a hash-verified source: ${normalizePath(source)}`, { cause: restoreError instanceof Error ? restoreError.message : String(restoreError) }); }
      if (hasCode(error, "EEXIST")) return conflict("target_exists", `Transition target already exists: ${normalizePath(target)}`);
...
function transitionJournalRoot(workspaceRoot: string): string {
  return path.join(workspaceRoot, ".pi-project-artifacts", "todo-transitions-v1");
}

async function writeTransitionJournal(workspaceRoot: string, runDirectory: string, journal: TodoTransitionJournal, replace: boolean): Promise<void> {
...
async function recoverPendingTransitions(roots: { workspaceRoot: string; todosRoot: string }, signal: AbortSignal): Promise<void> {
  ...
  let entries;
  try { entries = await readdir(journalRoot, { withFileTypes: true }); }
  catch (error) { if (hasCode(error, "ENOENT")) return; throw error; }
  for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
    throwIfAborted(signal);
    if (!entry.isDirectory() || entry.isSymbolicLink()) throw new ProjectArtifactError("transition_journal_invalid", `Unexpected transition journal entry: ${normalizePath(path.join(journalRoot, entry.name))}`);
    ...
    const journal = validateTransitionJournal(JSON.parse(await readFile(path.join(physicalRun, "journal.json"), "utf8")) as unknown);
    await recoverTransitionRun(roots, physicalRun, journal);
  }
}

function validateTransitionJournal(value: unknown): TodoTransitionJournal {
  const journal = value as TodoTransitionJournal;
...
async function recoverTransitionRun(roots: { workspaceRoot: string; todosRoot: string }, runDirectory: string, journal: TodoTransitionJournal): Promise<void> {
  ...
  if (sourceHash === undefined && targetHash === undefined) {
    ...
    await chmod(journal.source, journal.sourceMode).catch((error) => { if (process.platform !== "win32") throw error; });
    await fsyncDirectory(path.dirname(journal.source));
    await rm(runDirectory, { recursive: true, force: true });
    return;
  }
...
async function rollbackTransitionRun(roots: { workspaceRoot: string; todosRoot: string }, runDirectory: string, journal: TodoTransitionJournal): Promise<void> {
  const targetHash = await fileDigest(journal.target);
  if (targetHash !== undefined && targetHash !== journal.targetHash) throw new ProjectArtifactError("rollback_conflict", `Transition rollback refused externally changed target: ${normalizePath(journal.target)}`);
  ...
  const sourceHash = await fileDigest(journal.source);
  if (sourceHash !== undefined && sourceHash !== journal.sourceHash) throw new ProjectArtifactError("rollback_conflict", `Transition rollback refused externally changed source: ${normalizePath(journal.source)}`);
  if (sourceHash === undefined) {
    const backup = path.join(runDirectory, "source.original");
    if (await fileDigest(backup) !== journal.sourceHash) throw new ProjectArtifactError("rollback_backup_invalid", `Transition source backup is unavailable: ${normalizePath(backup)}`);
    await assertPhysicalContainment(roots.todosRoot, journal.source, "todo transition source");
...
async function fileDigest(file: string): Promise<string | undefined> {
  try { return hashBytes(await readFile(file)); }
  catch (error) { if (hasCode(error, "ENOENT")) return undefined; throw error; }
}

async function scanTodos(todosRoot: string): Promise<ScanResult> {
  let files: string[] = [];
  try { files = await listTodoMarkdown(todosRoot); }
  catch (error) { if (!hasCode(error, "ENOENT")) throw error; }
  const inspections = await Promise.all(files.map(async (file) => ({ path: file, inspected: await inspectTodoFile(file) })));
  const todos: TodoIdentityV1[] = [];
  const issues: TodoIssue[] = [];
  for (const { path: file, inspected } of inspections) {
    if (inspected.state === "valid" && inspected.todo !== undefined) todos.push(inspected.todo);
    else for (const issue of inspected.issues) issues.push({ path: file, code: issue.code, summary: issue.summary });
  }
  const duplicates = new Map<number, TodoIdentityV1[]>();
  const collisions = new Map<string, TodoIdentityV1[]>();
  ...
  for (const [id, matches] of duplicates) if (matches.length > 1) for (const todo of matches) { excluded.add(todo.path); issues.push({ path: todo.path, code: "duplicate_issue_id", summary: `Todo issue ID ${id} is duplicated; canonical tools will not choose an authority.` }); }
  for (const matches of collisions.values()) if (matches.length > 1) for (const todo of matches) { excluded.add(todo.path); issues.push({ path: todo.path, code: "path_collision", summary: "Todo path has a normalization/case-fold collision." }); }
...
export async function inspectTodoFile(file: string): Promise<Inspected> {
  let bytes: Buffer;
  try {
    const physical = await realpath(file);
    if (collisionKey(physical) !== collisionKey(file)) return { state: "invalid", issues: [{ code: "symlink_todo", summary: "Todo files may not be symlinks or junction aliases." }] };
  ...
  let filename;
  try { filename = parseCanonicalTodoBasenameV1(path.basename(file)); }
  catch { return { state: "invalid", issues: [{ code: "filename_invalid", summary: "Todo filename is not canonical." }], text: bytes.toString("utf8") }; }
  const text = bytes.toString("utf8");
  const parsed = parseMarkdown(text);
  const issues: TodoIssue[] = [];
  if (parsed.rawFrontmatter === undefined || parsed.malformed) issues.push({ code: "frontmatter_invalid", summary: "Todo frontmatter is missing, unterminated, or malformed." });
  ...
  if (!validFrontmatterId) issues.push({ code: "frontmatter_issue_id_invalid", summary: "Todo frontmatter issue_id must be a positive decimal identity." });
  if (!validFrontmatterStatus) issues.push({ code: status === "completed" ? "frontmatter_status_legacy" : "frontmatter_status_invalid", summary: status === "completed" ? "Legacy completed status is non-canonical; consuming projects own reconciliation." : "Todo frontmatter status is invalid." });
  if (!validFrontmatterPriority) issues.push({ code: "frontmatter_priority_invalid", summary: "Todo frontmatter priority is invalid." });
  let conflictFound = false;
  if (validFrontmatterId && issueId !== filename.issueId) { conflictFound = true; issues.push({ code: "issue_id_conflict", summary: "Filename and frontmatter contain different valid issue IDs." }); }
  if (validFrontmatterStatus && status !== filename.status) { conflictFound = true; issues.push({ code: "status_conflict", summary: "Filename and frontmatter contain different valid statuses." }); }
  if (validFrontmatterPriority && priority !== filename.priority) { conflictFound = true; issues.push({ code: "priority_conflict", summary: "Filename and frontmatter contain different valid priorities." }); }
  if (issues.length > 0) return { state: conflictFound ? "conflict" : "invalid", issues, text };
...
async function identityFromWrittenFile(file: string): Promise<TodoIdentityV1> {
  const inspected = await inspectTodoFile(file);
  if (inspected.state !== "valid" || inspected.todo === undefined) throw new ProjectArtifactError("post_write_invalid", `Written todo failed canonical validation: ${normalizePath(file)}`);
  return inspected.todo;
}
function preExistingConflict(scan: ScanResult): TodoLifecycleResultV1 | undefined {
  if (scan.issues.length === 0) return undefined;
...
async function listTodoMarkdown(root: string): Promise<string[]> {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      if (entry.isSymbolicLink()) {
        if (entry.name.toLowerCase().endsWith(".md")) output.push(path.join(current, entry.name));
        continue;
      }
      ...
      if (entry.isDirectory()) stack.push(child);
        ...
        if (!isPathInside(physicalRoot, physical)) throw new ProjectArtifactError("path_escape", `Todo escaped its physical root: ${normalizePath(child)}`);
...
async function directoryHash(root: string): Promise<string> {
  let entries: string[];
  try { entries = await listDirectoryEntries(root); }
  catch (error) { if (hasCode(error, "ENOENT")) entries = []; else throw error; }
...
async function listDirectoryEntries(root: string): Promise<string[]> {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (entry.isSymbolicLink()) output.push(`symlink\0${relative}`);
      else if (entry.isDirectory()) { output.push(`dir\0${relative}`); stack.push(child); }
      else if (entry.isFile()) output.push(`file\0${relative}\0${hashBytes(await readFile(child))}`);
      else output.push(`other\0${relative}`);
    }
  }
  return output.sort();
}
function normalizeTodoBody(title: string, body: string): string {
  const normalized = body.replace(/^\uFEFF/u, "");
  if (/^#\s+/mu.test(normalized)) return normalized.endsWith("\n") ? normalized : `${normalized}\n`;
  const prefix = `# ${title.trim()}\n\n`;
  const text = `${prefix}${normalized.replace(/^\s+/u, "")}`;
  return text.endsWith("\n") ? text : `${text}\n`;
}
function slugify(value: string): string { return value.normalize("NFKD").replace(/[\u0300-\u036f]/gu, "").toLowerCase().replace(/[^a-z0-9]+/gu, "-").replace(/^-+|-+$/gu, "").slice(0, 80).replace(/-+$/u, ""); }
function scalar(value: unknown): string | undefined { return typeof value === "string" ? value.trim().replace(/^['"]|['"]$/gu, "") : typeof value === "number" || typeof value === "boolean" ? String(value) : undefined; }
function numericId(value: unknown): number | undefined { const text = scalar(value); if (text === undefined || !/^\d+$/u.test(text)) return undefined; const number = Number(text); return Number.isSafeInteger(number) && number > 0 ? number : undefined; }
function renderId(issueId: number): string { return String(issueId).padStart(3, "0"); }
...
function lockOwner(todosRoot: string): Readonly<Record<string, unknown>> { return Object.freeze({ schema: "@aefree/pi-project-artifacts/lock", version: 1, owner: "todo-lifecycle", pid: process.pid, createdAt: new Date().toISOString(), todosRootHash: shortHash(todosRoot) }); }
function conflict(code: string, summary: string): TodoLifecycleResultV1 { return Object.freeze({ outcome: "conflict", code, summary }); }
function blocked(code: string, summary: string): TodoLifecycleResultV1 { return Object.freeze({ outcome: "blocked", code, summary }); }
