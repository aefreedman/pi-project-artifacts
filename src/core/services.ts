...
export function createArtifactSearchServiceV1(): ArtifactSearchServiceV1 {
  ...
  return Object.freeze({
    ...
    owner: OWNER,
    async search(context: ArtifactExecutionContextV1, request: ArtifactSearchRequestV1) {
      const binding = artifactExecutionBindingV1(context);
      if (binding === undefined) throw new Error("Artifact search execution context is not bound to the current Pi session.");
...
export function createTodoLifecycleServiceV1(options: TodoRuntimeOptions = {}): TodoLifecycleServiceV1 {
  return Object.freeze({
    contractVersion: 1,
    id: "project-file-todos.v1",
    kind: "todo-lifecycle-service",
    owner: OWNER,
    async execute(context: ArtifactExecutionContextV1, request: TodoLifecycleRequestV1) {
      const binding = artifactExecutionBindingV1(context);
      if (binding === undefined) throw new Error("Todo execution context is not bound to the current Pi session.");
