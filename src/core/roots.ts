...
export function resolveFrom(base: string, raw: string | undefined, fallback: string): string {
  if (raw === undefined || raw.trim() === "") return path.resolve(base, fallback);
...
export function resolveArtifactRoots(cwd: string, options: { workspaceRoot?: string; docsRoot?: string; todosRoot?: string }): ArtifactRoots {
 ...
 */
export async function physicalDirectory(target: string, create = false): Promise<string> {
  ...
  try {
    ...
    if (!physicalStats.isDirectory()) throw new Error("not a directory");
    return path.resolve(physical);
  } catch (error) {
    if (error instanceof ProjectArtifactError) throw error;
    const code = typeof error === "object" && error !== null && "code" in error ? String((error as { code?: unknown }).code) : undefined;
    if (code === "ENOENT") throw new ProjectArtifactError("root_missing", `Artifact root does not exist: ${normalizePath(target)}`, { path: normalizePath(target) });
...
export async function scopedPhysicalDirectory(physicalExecutionRoot: string, target: string, label: string): Promise<string> {
...
export async function scopedPhysicalDirectoryOrMissing(physicalExecutionRoot: string, target: string, label: string): Promise<string> {
  const contained = await assertPhysicalContainment(physicalExecutionRoot, target, label);
  try { return await scopedPhysicalDirectory(physicalExecutionRoot, contained, label); }
  catch (error) {
    if (error instanceof ProjectArtifactError && error.code === "root_missing") return contained;
    throw error;
  }
}
 ...
 */
export async function assertPhysicalContainment(physicalRoot: string, target: string, label = "path"): Promise<string> {
  ...
  while (true) {
    try {
      ...
      if (parent === cursor) throw new ProjectArtifactError("path_escape", `${label} has no existing contained ancestry: ${normalizePath(absolute)}`);
