...
export type ParsedMarkdown = Readonly<{
  ...
  frontmatterRange?: Readonly<{ start: number; end: number }>;
  malformed: boolean;
}>;
...
function splitInline(value: string): string[] {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if ((char === '"' || char === "'") && (index === 0 || value[index - 1] !== "\\")) quote = quote === char ? "" : quote === "" ? char : quote;
    if (char === "," && quote === "") {
...
export function parseMarkdown(text: string): ParsedMarkdown {
  const delimiter = /^---\r?\n/u.exec(text);
  if (delimiter === null) return Object.freeze({ frontmatter: Object.freeze({}), body: text, malformed: false });
  const closing = /\r?\n---(?:\r?\n|$)/gu;
  ...
  for (const line of lines) {
    ...
    if (item !== null && currentListKey !== undefined) {
      const list = Array.isArray(frontmatter[currentListKey]) ? frontmatter[currentListKey] as unknown[] : [];
    ...
    if (key === null) {
      ...
      if (!/^\s+/u.test(line)) malformed = true;
      continue;
    }
    const value = key[2] ?? "";
    if (value.trim() === "") {
...
export function extractTitle(body: string, frontmatter: Readonly<Record<string, unknown>>): string | undefined {
...
export function extractArtifactLinks(body: string): string[] {
  ...
  for (const expression of [/(?:^|[\s(])((?:docs|todos)\/[A-Za-z0-9_./ -]+?\.md)(?=$|[\s)\]`.,:;])/gimu, /\]\(([^)]+\.md)(?:#[^)]*)?\)/gimu]) {
    for (const match of body.matchAll(expression)) {
      ...
      if (normalized.startsWith("docs/") || normalized.startsWith("todos/")) output.add(normalized);
    }
  }
  return [...output].sort();
}

export function replaceTopLevelFrontmatterScalar(text: string, key: string, expected: string, replacement: string): string {
  ...
  if (matches.length !== 1) throw new ProjectArtifactError("todo_frontmatter_ambiguous", `Todo frontmatter must contain exactly one ${key} field.`);
  const current = matches[0]![2]!.trim().replace(/^['"]|['"]$/gu, "");
  if (current !== expected) throw new ProjectArtifactError("todo_frontmatter_conflict", `Todo frontmatter ${key} changed from the expected value.`);
...
export function serializeFrontmatter(metadata: Readonly<Record<string, unknown>>, order: readonly string[] = []): string {
...
function serializeKey(key: string, value: unknown, indent: number): string {
  const prefix = " ".repeat(indent);
  if (!/^[A-Za-z0-9_-]+$/u.test(key)) throw new ProjectArtifactError("frontmatter_key_invalid", `Frontmatter key cannot be serialized safely: ${key}`);
  if (Array.isArray(value)) {
    if (value.length === 0) return `${prefix}${key}: []`;
    if (value.every(isScalar)) return `${prefix}${key}: [${value.map(serializeScalar).join(", ")}]`;
