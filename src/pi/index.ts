...
const DESCRIBE_PARAMETERS = Type.Object({
  ...
  freshnessMode: Type.Optional(StringEnum(["auto", "strict", "memory"] as const)),
  freshnessTtlMs: Type.Optional(Type.Integer({ minimum: 0, default: 30000 })),
  outputMode: Type.Optional(StringEnum(["compact", "detailed"] as const, { description: "Compact lists all names/counts and always omits examples; detailed adds type/cardinality detail." })),
...
export default function registerProjectArtifacts(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, _onUpdate, ctx) {
      ...
      return { content: [{ type: "text" as const, text: formatArtifactWorkspaceDescription(result.details as Parameters<typeof formatArtifactWorkspaceDescription>[0]) }], details: result.details };
    },
  });
