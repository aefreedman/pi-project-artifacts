...
export const PROJECT_ARTIFACTS_CONTRACT_VERSION_V1 = 1;
export const ARTIFACT_PROFILE_REGISTRY_KEY_V1 = "@aefree/pi-project-artifacts/profiles/v1";
export const ARTIFACT_SEARCH_SERVICE_REGISTRY_KEY_V1 = "@aefree/pi-project-artifacts/search-services/v1";
export const TODO_LIFECYCLE_SERVICE_REGISTRY_KEY_V1 = "@aefree/pi-project-artifacts/todo-lifecycle-services/v1";
/** Frontmatter fields owned by the canonical todo writer rather than caller metadata. */
export const TODO_CANONICAL_FRONTMATTER_KEYS_V1 = Object.freeze(["issue_id", "status", "priority"]);
...
export function createArtifactProfileRegistryV1() {
...
export function createArtifactSearchServiceRegistryV1() {
    return createCapabilityRegistry({
...
export function createTodoLifecycleServiceRegistryV1() {
    return createCapabilityRegistry({
...
export function resolveArtifactProfilesV1(scope, registry = createArtifactProfileRegistryV1()) {
    return resolveContracts(scope, registry, false);
}
export function resolveArtifactSearchServiceV1(scope, registry = createArtifactSearchServiceRegistryV1()) {
    return resolveContracts(scope, registry, true);
}
export function resolveTodoLifecycleServiceV1(scope, registry = createTodoLifecycleServiceRegistryV1()) {
    return resolveContracts(scope, registry, true);
}
export function assertArtifactExecutionContextV1(value) {
    const context = asObject(value, "ArtifactExecutionContextV1");
    assertNonEmpty(context.cwd, "ArtifactExecutionContextV1.cwd");
    if (context.requestId !== undefined)
        assertNonEmpty(context.requestId, "ArtifactExecutionContextV1.requestId");
    if (context.signal === null || typeof context.signal !== "object" || typeof context.signal.aborted !== "boolean" || typeof context.signal.addEventListener !== "function")
        fail("ArtifactExecutionContextV1.signal must be an AbortSignal");
}
export function assertArtifactProfileV1(value) {
    ...
    if (!Array.isArray(record.fields) || !Array.isArray(record.validators))
        fail("ArtifactProfileV1 fields/validators must be arrays");
    ...
    for (const value of record.fields) {
        ...
        if (fieldNames.has(field.name))
            fail(`duplicate artifact field '${field.name}'`);
        fieldNames.add(field.name);
        if (!["string", "string_list", "integer", "boolean", "date"].includes(String(field.type)))
            fail("invalid artifact field type");
        if (typeof field.indexed !== "boolean" || typeof field.filterable !== "boolean")
            fail("artifact field flags must be boolean");
        if (field.enumValues !== undefined)
            assertStringArray(field.enumValues, "ArtifactFieldDefinitionV1.enumValues");
    }
    ...
    for (const value of record.validators) {
        ...
        if (validatorIds.has(validator.id))
            fail(`duplicate artifact validator '${validator.id}'`);
        validatorIds.add(validator.id);
        if (typeof validator.validate !== "function")
            fail("ArtifactValidatorV1.validate must be a function");
    }
    if (record.appliesTo !== undefined && typeof record.appliesTo !== "function")
        fail("ArtifactProfileV1.appliesTo must be a function");
}
export function assertArtifactValidationResultV1(value) {
    ...
    if (result.outcome === "unavailable" || result.outcome === "error") {
        ...
        if (typeof result.retryable !== "boolean")
            fail("retryable must be boolean");
        return;
    }
    if ((result.outcome !== "invalid" && result.outcome !== "conflict") || !Array.isArray(result.issues))
        fail("invalid validation outcome");
...
export function assertArtifactSearchServiceV1(value) {
    const record = assertContractRecord(value, "artifact-search-service");
    if (typeof record.search !== "function")
        fail("ArtifactSearchServiceV1.search must be a function");
}
export function assertTodoLifecycleServiceV1(value) {
    const record = assertContractRecord(value, "todo-lifecycle-service");
    if (typeof record.execute !== "function")
        fail("TodoLifecycleServiceV1.execute must be a function");
}
export function assertArtifactSearchResultV1(value) {
    const result = asObject(value, "ArtifactSearchResultV1");
    if (typeof result.text !== "string")
        fail("ArtifactSearchResultV1.text must be a string");
    asObject(result.details, "ArtifactSearchResultV1.details");
    assertArtifactExecutionProvenanceV1(result.provenance);
}
export function assertArtifactExecutionProvenanceV1(value) {
    const provenance = asObject(value, "ArtifactExecutionProvenanceV1");
    if (provenance.schema !== "@aefree/pi-project-artifacts/execution-provenance" || provenance.version !== 1)
        fail("artifact provenance schema/version mismatch");
    ...
    if (canonical.contractVersion !== 1)
        fail("canonical.contractVersion must be 1");
    if (!Array.isArray(provenance.profiles) || !Array.isArray(provenance.fallbacks))
        fail("artifact provenance arrays are required");
    if (provenance.executionGate !== "executed" && provenance.executionGate !== "blocked")
        fail("artifact execution gate is invalid");
}
/**
 ...
 */
export function canonicalizeTodoAbsolutePathV1(value, label = "todo path") {
    ...
    if (style === "posix") {
        if (input.includes("\\"))
            fail(`${label} must not mix POSIX and Windows separators`);
        return trimTodoPathTrailingSeparator(posixPath.normalize(input), style);
    }
    if (style === "windows-unc" && /^[\\/]{2}[?.][\\/]/u.test(input))
        fail(`${label} must not use a Windows device namespace`);
...
export function resolveTodoPathContextV1(workspaceRoot, todosRoot) {
...
export function todoPathCollisionKeyV1(value) {
...
export function assertTodoPathContainedV1(value, context, label = "todo path") {
    ...
    if (candidateStyle !== validatedContext.style)
        fail(`${label} must use the todosRoot path style`);
    ...
    if (!compared.startsWith(prefix))
        fail(`${label} must be contained under todosRoot`);
    return candidate;
}
/** Parses and validates `{renderedId}-{status}-{priority}-{description}.md`. */
export function parseCanonicalTodoBasenameV1(value) {
    assertNonEmpty(value, "todo basename");
    if (value.includes("/") || value.includes("\\"))
        fail("todo basename must not contain path separators");
    const match = /^(\d{3,})-(pending|ready|complete)-(p1|p2|p3)-([a-z0-9]+(?:-[a-z0-9]+)*)\.md$/u.exec(value);
    if (match === null)
        fail("todo basename must use the canonical id-status-priority-description.md form");
...
export function parseCanonicalTodoPathV1(value) {
    const canonical = canonicalizeTodoAbsolutePathV1(value);
    const style = todoPathStyleV1(canonical, "todo path");
    const implementation = style === "posix" ? posixPath : win32Path;
...
export function assertTodoLifecycleRequestV1(value) {
    ...
    switch (request.operation) {
        ...
        case "allocate_id":
            ...
            if (hasOwn(request, "expectedDirectoryHash"))
                assertSha256(request.expectedDirectoryHash, "TodoLifecycleRequestV1.allocate_id.expectedDirectoryHash");
            return;
        case "create": {
            ...
            if (request.exclusive !== true)
                fail("todo creation must be exclusive");
            if (hasOwn(request, "expectedDirectoryHash"))
                assertSha256(request.expectedDirectoryHash, "TodoLifecycleRequestV1.create.expectedDirectoryHash");
            ...
            if (typeof todo.body !== "string")
                fail("TodoDocumentInputV1.body must be a string");
            if (hasOwn(todo, "frontmatter"))
                assertTodoFrontmatterV1(todo.frontmatter, status, todo.priority);
            return;
        }
        case "transition": {
            ...
            if (request.toStatus === current.status)
                fail("TodoLifecycleRequestV1.transition.toStatus must change status");
...
export function assertTodoLifecycleResultV1(value) {
    ...
    switch (result.outcome) {
        case "inspected":
            ...
            if (result.state !== "valid" && result.state !== "invalid" && result.state !== "conflict")
                fail("TodoLifecycleResultV1.inspected.state is invalid");
            if (hasOwn(result, "todo"))
                assertTodoIdentityV1(result.todo);
            assertTodoIssuesV1(result.issues, "TodoLifecycleResultV1.inspected.issues", false);
            if (result.state === "valid" && !hasOwn(result, "todo"))
                fail("a valid inspected result requires todo identity");
            if (result.state === "valid" && result.issues.length !== 0)
                fail("a valid inspected result must not contain issues");
            if (result.state !== "valid" && result.issues.length === 0)
                fail("an invalid/conflict inspected result requires issues");
            return;
        case "listed": {
            assertOnlyKeys(result, ["outcome", "todos", "issues"], "TodoLifecycleResultV1.listed");
            if (!Array.isArray(result.todos))
                fail("TodoLifecycleResultV1.listed.todos must be an array");
            ...
            for (const todo of result.todos) {
                ...
                if (paths.has(pathKey))
                    fail("TodoLifecycleResultV1.listed contains a normalized/case-fold path collision");
                paths.add(pathKey);
                if (issueIds.has(identity.issueId))
                    fail(`TodoLifecycleResultV1.listed contains duplicate issueId ${identity.issueId}`);
        ...
        case "allocated":
            ...
            assertRenderedTodoIdV1(result.renderedId, result.issueId, "TodoLifecycleResultV1.allocated.renderedId");
        ...
        case "blocked":
            ...
            if (hasOwn(result, "currentContentHash"))
                assertSha256(result.currentContentHash, `TodoLifecycleResultV1.${result.outcome}.currentContentHash`);
            return;
        default:
...
export function assertTodoLifecycleResultForRequestV1(value, requestValue) {
    ...
    if (result.outcome === "conflict" || result.outcome === "blocked")
        return;
    ...
    if (result.outcome !== expectedOutcome)
        fail(`todo ${request.operation} cannot return ${result.outcome}`);
    switch (request.operation) {
        case "inspect": {
            ...
            if (inspected.todo !== undefined) {
                ...
                if (!todoPathsEquivalentV1(inspected.todo.path, request.path, context.style))
                    fail("inspected todo path must match the requested path");
            }
            return;
        }
        case "list": {
            const listed = result;
            for (const todo of listed.todos)
                assertTodoIdentityInContextV1(todo, context, "TodoLifecycleResultV1.listed.todo");
            for (const issue of listed.issues) {
                if (issue.path !== undefined)
                    assertTodoPathContainedV1(issue.path, context, "TodoLifecycleResultV1.listed.issue.path");
            }
            return;
        }
        case "allocate_id":
        ...
        case "create": {
            ...
            if (created.todo.status !== expectedStatus)
                fail("created todo status must match the create request");
            if (created.todo.priority !== request.todo.priority)
                fail("created todo priority must match the create request");
            return;
        }
        case "transition": {
            ...
            if (!todoPathsEquivalentV1(transitioned.previousPath, request.path, context.style))
                fail("transition previousPath must match the requested path");
            const previous = parseCanonicalTodoPathV1(request.path);
            const next = parseCanonicalTodoPathV1(transitioned.todo.path);
            if (transitioned.todo.status !== request.toStatus || next.status !== request.toStatus)
                fail("transitioned todo status must match toStatus");
            ...
            if (!todoPathsEquivalentV1(transitioned.todo.path, expectedPath, context.style))
                fail("transitioned todo path does not match canonical rename semantics");
            return;
        }
    }
}
function assertTodoIdentityV1(value) {
    ...
    assertRenderedTodoIdV1(todo.renderedId, todo.issueId, "TodoIdentityV1.renderedId");
...
function assertTodoFrontmatterV1(value, status, priority) {
    ...
    for (let index = 0; index < normalizedEntries.length; index += 1) {
        ...
        if (entry.normalized === "")
            fail(`TodoDocumentInputV1.frontmatter key '${entry.key}' is invalid`);
        ...
        if (entry.key !== entry.normalized && TODO_CANONICAL_FRONTMATTER_KEYS_V1.includes(entry.normalized)) {
            fail(`TodoDocumentInputV1.frontmatter key '${entry.key}' conflicts with reserved '${entry.normalized}'`);
        }
        if (TODO_FORBIDDEN_FRONTMATTER_KEYS_V1.includes(entry.normalized)) {
            fail(`TodoDocumentInputV1.frontmatter key '${entry.key}' is reserved`);
        }
        if (entry.normalized === "status" && metadata[entry.key] !== status)
            fail("frontmatter status must exactly match the top-level status");
        if (entry.normalized === "priority" && metadata[entry.key] !== priority)
            fail("frontmatter priority must exactly match the top-level priority");
    }
    assertTodoMetadataValueV1(metadata, "TodoDocumentInputV1.frontmatter", new Set());
}
function normalizeTodoFrontmatterKeyV1(value) {
...
function assertTodoMetadataValueV1(value, label, ancestors) {
    if (value === null || typeof value === "string" || typeof value === "boolean")
        return;
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            fail(`${label} numbers must be finite`);
        return;
    }
    if (typeof value !== "object")
        fail(`${label} contains a non-data value`);
    if (ancestors.has(value))
        fail(`${label} must not contain cycles`);
    ancestors.add(value);
    if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1)
            assertTodoMetadataValueV1(value[index], `${label}[${index}]`, ancestors);
    }
    else {
        assertPlainRecord(value, label);
        for (const key of assertEnumerableDataKeysV1(value, label)) {
            assertTodoMetadataValueV1(value[key], `${label}.${key}`, ancestors);
        }
    }
    ancestors.delete(value);
}
function assertEnumerableDataKeysV1(value, label) {
...
function assertTodoIssuesV1(value, label, allowPath) {
    if (!Array.isArray(value))
    ...
    for (const entry of value) {
        ...
        if (hasOwn(issue, "path"))
            assertAbsolutePath(issue.path, `${label}.path`);
    }
}
function assertTodoPathContextV1(value) {
    ...
    if (workspaceRoot !== context.workspaceRoot || todosRoot !== context.todosRoot)
        fail("TodoPathContextV1 roots must be canonical");
    const style = todoPathStyleV1(workspaceRoot, "TodoPathContextV1.workspaceRoot");
    if (style !== todoPathStyleV1(todosRoot, "TodoPathContextV1.todosRoot") || context.style !== style)
        fail("TodoPathContextV1 path styles must match");
    return context;
}
function todoPathStyleV1(value, label) {
...
function trimTodoPathTrailingSeparator(value, style) {
    const implementation = style === "posix" ? posixPath : win32Path;
...
function assertRenderedTodoIdV1(value, issueId, label) {
    if (typeof value !== "string" || !/^\d{3,}$/.test(value))
        fail(`${label} must contain at least three decimal digits`);
    if (value !== String(issueId).padStart(3, "0"))
        fail(`${label} must match issueId`);
}
function assertTodoStatusV1(value, label) {
    if (value !== "pending" && value !== "ready" && value !== "complete")
        fail(`${label} is invalid`);
}
function assertTodoPriorityV1(value, label) {
...
function assertPlainRecord(value, label) {
    ...
    if (prototype !== Object.prototype && prototype !== null)
        fail(`${label} must be a record object`);
}
...
function resolveContracts(scope, registry, exclusive) {
    ...
    if (duplicate)
        return Object.freeze({ outcome: "duplicate", code: "duplicate_registration", expectedContractVersion: 1, registryKey: registry.registryKey, providerIds: Object.freeze([...new Set(ids)]), catalog });
    if (records.length > 0)
        return Object.freeze({ outcome: "available", records, catalog });
    const incompatible = catalog.status === "incompatible";
    return Object.freeze({ outcome: incompatible ? "incompatible" : "missing", code: incompatible ? "incompatible_contract" : "missing_registration", expectedContractVersion: 1, registryKey: registry.registryKey, providerIds: Object.freeze(incompatibleCatalogIds(catalog)), catalog });
}
function incompatibleCatalogIds(catalog) {
...
function assertContractRecord(value, kind) {
    const record = asObject(value, kind);
    if (record.contractVersion !== 1 || record.kind !== kind)
        fail(`${kind} contractVersion/kind mismatch`);
