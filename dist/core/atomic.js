...
export async function withMutationQueue(key, task) {
    ...
    try {
        ...
        release();
        if (mutationQueues.get(canonical) === tail)
            mutationQueues.delete(canonical);
    }
}
export async function acquireDirectoryLock(lockPath, owner, options = {}) {
    ...
    while (true) {
        ...
        try {
            ...
            try {
                ...
                if (hasCode(readError, "ENOENT")) {
                    if (Date.now() - started > timeoutMs)
                        throw new ProjectArtifactError("lock_owner_missing", `Lock owner metadata is missing: ${normalizePath(lockPath)}`, { lockPath: normalizePath(lockPath) });
                ...
                if (readError instanceof SyntaxError) {
                    ...
                    if (malformedOwnerObservation === malformed)
                        throw new ProjectArtifactError("lock_owner_malformed", `Lock owner metadata is malformed: ${normalizePath(ownerPath)}`, { lockPath: normalizePath(lockPath) });
            ...
            if (!isOwnedLock(lockOwner))
                throw new ProjectArtifactError("lock_owner_incompatible", `Lock is not owned by pi-project-artifacts: ${normalizePath(lockPath)}`, { lockPath: normalizePath(lockPath) });
            ...
            if (Date.now() - started > timeoutMs)
                throw new ProjectArtifactError("lock_timeout", `Timed out waiting for lock: ${normalizePath(lockPath)}`, { lockPath: normalizePath(lockPath) });
            await delay(retryMs, options.signal);
            continue;
        }
        const ownerPath = path.join(lockPath, "owner.json");
        try {
            ...
            return async () => {
                ...
                if (!released)
                    throw new ProjectArtifactError("lock_release_raced", `Lock release lost a race: ${normalizePath(lockPath)}`, { lockPath: normalizePath(lockPath) });
            };
        }
        catch (error) {
            const current = await readLockOwner(ownerPath).catch(() => undefined);
            if (isOwnedLock(current) && current.nonce === nonce)
                await quarantineOwnedLock(lockPath, current, options.physicalRoot, "failed-acquire", retryMs, options.signal).catch(() => undefined);
            throw error;
        }
    }
}
export async function withDirectoryLock(lockPath, owner, task, options = {}) {
...
export async function exclusiveWrite(target, content) {
...
export async function atomicReplace(target, content, options = {}) {
...
export async function removeIfMatches(target, expected) {
    try {
...
export async function writeJsonAtomic(target, value, validatedExisting, failureInjector) {
...
export async function fsyncDirectory(directory) {
    ...
    try {
        ...
        if (process.platform !== "win32" && !hasCode(error, "EINVAL") && !hasCode(error, "EPERM"))
            throw error;
    }
    finally {
        await handle?.close().catch(() => undefined);
...
export async function cleanupOwnedOrphanTemps(indexPath, staleMs = DEFAULT_STALE_MS) {
    const directory = path.dirname(indexPath);
    const prefix = `.${path.basename(indexPath)}.`;
    const removed = [];
    let entries;
    try {
        entries = await import("node:fs/promises").then(({ readdir }) => readdir(directory, { withFileTypes: true }));
    }
    catch (error) {
        if (hasCode(error, "ENOENT"))
            return removed;
    ...
    for (const entry of entries) {
        ...
        if (Date.now() - targetStats.mtimeMs <= staleMs)
            continue;
        await unlink(target);
        removed.push(target);
    }
...
async function quarantineOwnedLock(lockPath, expected, physicalRoot, reason, retryMs, signal) {
    ...
    try {
        ...
        for (let attempt = 0;; attempt += 1) {
            ...
            if (!await lockPathHasExpectedOwner(lockPath, expected))
                return false;
            try {
                ...
                if (hasCode(error, "ENOENT"))
                    return false;
                if (!hasCode(error, "EPERM") && !hasCode(error, "EBUSY"))
                    throw error;
                ...
                if (!await lockPathHasExpectedOwner(lockPath, expected))
                    return false;
                if (attempt >= QUARANTINE_RENAME_RETRIES)
                    return false;
        ...
        if (reclaiming)
            await rm(reclaimGuard, { recursive: true, force: true }).catch(() => undefined);
    }
}
async function lockPathHasExpectedOwner(lockPath, expected) {
    try {
...
async function revalidateLockPath(physicalRoot, target, label) {
    if (physicalRoot !== undefined)
        await assertPhysicalContainment(physicalRoot, target, label);
}
function isOwnedLock(value) {
    return typeof value === "object" && value !== null
        && value.schema === "@aefree/pi-project-artifacts/lock"
        && value.version === 1
...
function randomSuffix() { return `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`; }
