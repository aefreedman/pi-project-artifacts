...
const SEARCH_PARAMETERS = Type.Object({
    ...
    searchFields: Type.Optional(Type.Array(StringEnum(["path", "title", "tags", "frontmatter", "headings", "body"]))),
    ...
    scopes: Type.Optional(Type.Array(StringEnum(["all", "docs", "solutions", "plans", "memories", "todos"]))),
    ...
    rankProfile: Type.Optional(StringEnum(["balanced", "frontmatter", "recency", "todos"])),
    matchMode: Type.Optional(StringEnum(["all", "any", "phrase"])),
    ...
    outputMode: Type.Optional(StringEnum(["compact", "detailed"])),
...
export default function registerProjectArtifacts(pi) {
    ...
    pi.on("session_shutdown", (_event, ctx) => {
        ...
        if (activeScope !== ctx.sessionManager)
            return;
    ...
    pi.registerTool({
        ...
        async execute(toolCallId, params, signal, onUpdate, ctx) {
            ...
            const result = await service.search(executionContext(ctx, signal, toolCallId), params);
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            priority: StringEnum(["p1", "p2", "p3"]),
            status: Type.Optional(StringEnum(["pending", "ready", "complete"])),
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...ROOT_PARAMETERS,
            path: Type.String(),
            toStatus: StringEnum(["pending", "ready", "complete"]),
        ...
        async execute(_id, params, signal, _onUpdate, ctx) {
            ...
            const request = { operation: "transition", ...root, path: source, toStatus: params.toStatus, expectedContentHash: params.expectedContentHash };
...
function registerTodoReadTool(pi, name, label, description, operation, fields) {
    pi.registerTool({
        name,
        label,
        description,
...
function renderArtifactSearchResult(result, options, theme, context) {
...
function renderArtifactDescribeResult(result, options, theme, context) {
    const details = asRecord(result.details);
...
function renderTodoResult(result, options, theme, context) {
...
function expandableToolResult(result, options, theme, summary, color = "success", context) {
    const fullText = result.content.flatMap((entry) => entry.type === "text" ? [entry.text] : []).join("\n");
    if (options.isPartial || options.expanded)
        return new Text(fullText || summary, 0, 0);
...
function todoBasename(value) {
    const todo = asRecord(value);
    return typeof todo.path === "string" ? path.basename(todo.path) : "todo";
}
function cacheStateLabel(value) {
    if (value === "auto_fast_path")
        return "cache reused";
    if (value === "memory_fast_path")
        return "memory cache reused";
    if (value === "validated_unchanged")
        return "cache validated";
    if (value === "rebuilt")
        return "cache rebuilt";
    return undefined;
}
function requireSearchService(ctx) {
    const resolved = resolveArtifactSearchServiceV1(ctx.sessionManager);
    if (resolved.outcome !== "available")
        throw new ProjectArtifactError(resolved.code, "Canonical ArtifactSearchServiceV1 is unavailable. Install/enable @aefree/pi-project-artifacts and start a fresh session.", { registryKey: resolved.registryKey, providerIds: resolved.providerIds });
    return resolved.records[0];
}
function requireTodoService(ctx) {
    const resolved = resolveTodoLifecycleServiceV1(ctx.sessionManager);
    if (resolved.outcome !== "available")
        throw new ProjectArtifactError(resolved.code, "Canonical TodoLifecycleServiceV1 is unavailable. Install/enable @aefree/pi-project-artifacts and start a fresh session.", { registryKey: resolved.registryKey, providerIds: resolved.providerIds });
    return resolved.records[0];
}
function executionContext(ctx, signal, requestId) {
...
function normalizedRootParams(ctx, params) {
