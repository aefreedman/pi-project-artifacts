...
2. Require `WORKSPACE_ROOT`, `DOCS_ROOT`, and `TODOS_ROOT` to resolve physically at or below the session-approved current working directory; reject parent, sibling, and symlink/junction escapes before locks or content I/O.
3. Preserve nested child workspaces when the approved current directory is a coordination root.
4. Use physical paths via `DOCS_ROOT` and `TODOS_ROOT` in templates and outputs.
5. Map any `docs/...` or `todos/...` references to physical roots before reporting paths.
6. If operating from a coordination root, never create unresolved root-level
   `docs/` or `todos/` directories.
7. Validate resolved workspace markers before writing.

## Safety Checks Before Writes

- Target must be under `DOCS_ROOT` or `TODOS_ROOT`, and every selected root must remain physically contained by the session-approved current working directory.
